        ...
        return function () {
            $this->name('laravel-mail.')->namespace('\LaravelCompany\Mail\Http\Controllers')->group(static function (
                ...
                $appRouter->name('webview.')->prefix('webview')->namespace('Webview')->group(static function (
                    ...
                    $webviewRouter->get('record-invisible-pixel/{messageHash}', 'WebviewController@recordInvisiblePixel')->name('record-invisible-pixel');
                    //
                    $webviewRouter->get('track/{messageHash}/{url}', 'WebviewController@recordClickAndRedirect')->name('record-click-link');
        ...
        return function () {
            $this->name('laravel-mail.')->namespace('\LaravelCompany\Mail\Http\Controllers')->group(static function (
                ...
                $appRouter->name('campaigns.')->prefix('campaigns')->namespace('Campaigns')->group(static function (
                    ...
                    $campaignRouter->get('generate', 'CampaignsController@generate')->name('generate');
                    ...
                    $campaignRouter->post('rewrite', 'CampaignsController@rewrite')->name('rewrite');
                ...
                $appRouter->name('email_services.')->prefix('email-services')->namespace('EmailServices')->group(static function (
                    ...
                    $servicesRouter->get('verify', 'EmailServicesController@verify')->name('verify');
                    $servicesRouter->post('postVerify', 'EmailServicesController@postVerify')->name('postVerify');
               ...
               // $appRouter->resource('mailbox', 'Inbox\InboxController');
               $appRouter->name('mailbox.')->prefix('mailbox')->group(static function (Router $messageRouter) {

                    $messageRouter->get('/{folder}', 'Inbox\InboxController@folder')->name('folder');
                    $messageRouter->get('/{folder}/{messageId}', 'Inbox\InboxController@message')->name('message');
               });
                ...
                $appRouter->name('subscribers.')->prefix('subscribers')->namespace('Subscribers')->group(static function (
                    ...
                    $subscriberRouter->get('unsubscribe/{subscriberId}', 'SubscribersController@unsubscribe')->name('unsubscribe');

                    $subscriberRouter->get('enrich/{subscriberId}', 'SubscribersController@enrich')->name('enrich');

                    $subscriberRouter->get('validate', 'SubscribersController@validateSubscribers')->name('validate');
