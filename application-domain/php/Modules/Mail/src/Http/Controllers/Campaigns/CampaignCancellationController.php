...
class CampaignCancellationController extends Controller
{
    ...
    {
        $campaign = $this->campaignRepository->find(LaravelMail::currentWorkspaceId(), $campaignId, ['status']);

        return view('laravel-mail::campaigns.cancel', [
            'campaign' => $campaign,
        ]);
    }
    ...
    public function cancel(int $campaignId)
    {
        ...
        if (! $campaign->canBeCancelled()) {
            ...
            ])->redirectTo(route('laravel-mail.campaigns.index'));
        }
        ...
        return redirect()->route('laravel-mail.campaigns.index')->with([
