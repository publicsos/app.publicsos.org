...
use LaravelCompany\Mail\Services\Validation\ValidationService;
...
class CreateMessages
{
    ...
    protected function handleTag(Campaign $campaign, Tag $tag): void
    {
        ...
        $tag->subscribers()->whereNull('unsubscribed_at')->chunkById(1000, function ($subscribers) use ($campaign) {
            info('Sending tag status to subscribers' . $campaign->id);
            $this->dispatchToSubscriber($campaign, $subscribers);
        }, 'subscribers.id');
    }
    ...
    {
        // If a message already exists, then we'are going to assume that
        // it has already been dispatched. This makes the dispatch fault-tolerant
        // and prevents dispatching the same message to the same subscriber
    ...
    protected function dispatchNow(Campaign $campaign, Subscriber $subscriber): Message
    {
        ...
        if ($message = $this->findMessage($campaign, $subscriber)) {
            //Log::info('Message has previously been created campaign=' . $campaign->id . ' subscriber=' . $subscriber->id, 'laravel-mail');

            return $message;
        }

        // the message doesn't exist, so we'll create and dispatch
        //Log::info('Saving empty email message campaign=' . $campaign->id . ' subscriber=' . $subscriber->id, 'laravel-mail');
