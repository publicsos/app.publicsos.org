...
class CampaignCancellationController extends Controller
{
    ...
    {
        $campaign = $this->campaignRepository->find($this->workspaceID, $campaignId, ['status']);

        return view('mail::backend.campaigns.cancel', [
            'campaign' => $campaign,
        ]);
    }
    ...
    public function cancel(int $campaignId)
    {
        ...
        if (! $campaign->canBeCancelled()) {
            ...
            ])->redirectTo(route('backend.campaigns.index'));
        }
        ...
        return redirect()->route('backend.campaigns.index')->with([
