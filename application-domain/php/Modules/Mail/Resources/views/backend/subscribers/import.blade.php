    ...
    <div class="container-fluid">
        ...
        @component('mail::backend.layouts.partials.card')
            ...
            @slot('cardBody')
                ...
                <form action="{{ route('backend.subscribers.import.store') }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                    @csrf

                    <x-file-field name="file" :label="__('File')" required="required" />

                    <x-select-field name="tags[]" :label="__('Tags')" :options="$tags" multiple />

                    <div class="form-group row">
                        <div class="offset-sm-3 col-sm-9">
                            <a href="{{ route('backend.subscribers.index') }}" class="btn btn-light">{{ __('Back') }}</a>
