    ...
    @if($templates->isEmpty())
        <div class="col-12">
            <div class="text-center alert alert-warning">
                Nu au fost găsite șabloane pentru "<span class="highlight">{{ $searchTerm }}</span>"
            </div>
        </div>
    @else
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="gap-2 mt-2 d-flex justify-content-start">
                            ...
                            <a href="{{ route('backend.templates.edit', $template->id) }}" class="btn btn-block btn-primary">
                               <i class="fa fa-pencil"></i> {{ __('Edit') }}
                            </a>

                            @if (! $template->is_in_use)
                                <form action="{{ route('backend.templates.destroy', $template->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div class="card-body" style="max-height: 25vh; overflow: hidden;">
                        <a href="{{ route('backend.templates.edit', $template->id) }}">
                            <div class="text-white">
                                <img
