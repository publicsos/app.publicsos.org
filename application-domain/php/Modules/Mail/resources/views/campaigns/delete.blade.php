...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('laravel-mail.campaigns.destroy', $campaign->id) }}" method="post">
                @csrf
                @method('DELETE')
                <input type="hidden" name="id" value="{{ $campaign->id }}">
                <a href="{{ route('laravel-mail.campaigns.index') }}" class="btn btn-md btn-light">{{ __('Cancel') }}</a>
