...
@section('content')
    ...
    <script>
        class Drawflow {
            ...
            getNode(id){
                return this.drawflow.drawflow[this.module].data[id];
            }

            getNodeByData(key, value){

                for(var i in this.drawflow.drawflow[this.module].data){
                    if(this.drawflow.drawflow[this.module].data[i].data[key] == value){
                        return this.drawflow.drawflow[this.module].data[i];
                    }
                }
            }

            setData(id, key, value){
                this.drawflow.drawflow[this.module].data[id].data[key] = value;
            }
            ...
            addConnection(input_id, output_id, input_class, output_class){
                ...
                connection.classList.add("node_in_" + input_id);
                connection.classList.add("node_out_" + output_id);
                ...
                this.updateConnectionNodes(input_id);
            }
            ...
            addNode(id, name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false) {
                ...
                if (nodeId == 0) {
                    nodeId = this.nodeId;
                    nodeId = Math.floor(Math.random() * 10000) + 1000;
                }
                ...
                Object.entries(data).forEach(function (key, value) {
                    if (typeof key[1] === "object") {
                        ...
                        node.setAttribute("data-"+key[0], key[1]);
                ...
                this.drawflow.drawflow[this.module].data[nodeId] = json;
            ...
            removeNodeId(id) {
                this.dispatch('nodeBeforeRemoved', id.slice(5));
            ...
            removeConnection() {
                if (this.connection_selected != null) {
                    ...
                    this.dispatch('connectionRemoved', {
                        ouput_id: listclass[2].slice(14),
                        input_id: listclass[1].slice(13),
                        ouput_class: listclass[3],
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10" style="height: 90vh; overflow: hidden;">
                <div class="">
                ...
                <script>
                    ...
                    var new_node = `@include('laravel-mail::workflows.layouts.task_node_html', ['elementName' => $task->getTranslation(), 'element' => $task, 'type' => 'task', 'icon' => $task::$icon])`;
                    ...
                    var new_node = `@include('laravel-mail::workflows.layouts.task_node_html', ['elementName' => $trigger->getTranslation(), 'element' => $trigger, 'type' => 'trigger', 'icon' => $trigger::$icon])`;
                    ...
                    function loadResourceIntelligence(element_id, type, value, field_name, element) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.loadResourceIntelligence', ['workflow' => $workflow]) }}",
                            dataType: 'json',
                            data: {
                                _token: "{{ csrf_token() }}",
                                'resource': resource,
                    ...
                    function saveConditions(id, type) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.changeConditions', ['workflow' => $workflow]) }}",
                            data: {
                                _token: "{{ csrf_token() }}",
                                'data': data,
                                'type': type,
                    ...
                    function saveFields(id, type) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.changeValues', ['workflow' => $workflow]) }}",
                            data: {
                                _token: "{{ csrf_token() }}",
                                'data': data,
                                'type': type,
                    ...
                    editor.on('nodeCreated', function (node) {
                        ...
                        switch (node.data.type) {
                            case 'task':
                                $.ajax({
                                    type: "POST",
                                    url: "{{ route('laravel-mail.workflows.addTask', ['workflow' => $workflow]) }}",
                                    data: {
                                        _token: "{{ csrf_token() }}",
                                        node: node,
                                    },
                            ...
                            case 'trigger':
                                $.ajax({
                                    type: "POST",
                                    url: "{{ route('laravel-mail.workflows.addTrigger', ['workflow' => $workflow]) }}",
                                    data: {
                                        _token: "{{ csrf_token() }}",
                                        node: node,
                                    },
                    ...
                    editor.on('nodeBeforeRemoved', function (id) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.removeTask', ['workflow' => $workflow]) }}",
                            data: {
                                _token: "{{ csrf_token() }}",
                                node: node
                            },
                    ...
                    function updateNodePosition(node) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.updateNodePosition', ['workflow' => $workflow]) }}",
                            data: {
                                _token: "{{ csrf_token() }}",
                                node: node
                            },
                    ...
                    editor.on('connectionCreated', function (connection) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.addConnection', ['workflow' => $workflow]) }}",
                            data: {
                    ...
                    editor.on('connectionRemoved', function (connection) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.removeConnection', ['workflow' => $workflow]) }}",
                            data: {
                    ...
                    function addNodeToDrawFlow(name, pos_x, pos_y) {
                        ...
                        switch (name) {
                            @foreach(config('workflows.tasks') as $taskName => $taskClass)
                            case '{{ $taskName }}':
                                var {{$taskName}}_new_node = `@include('laravel-mail::workflows.layouts.task_node_html', ['elementName' => $taskClass::getTranslation(), 'fields' => $taskClass::$fields, 'type' => 'task', 'icon' => $taskClass::$icon])`;
                            ...
                            case '{{$triggerName}}':
                                var {{$taskName}}_new_node = `@include('laravel-mail::workflows.layouts.task_node_html', ['elementName' => $triggerClass::getTranslation(), 'fields' => $triggerClass::$fields, 'type' => 'trigger', 'icon' => $triggerClass::$icon])`;
                    ...
                    function loadLogs() {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.getLogs', ['workflow' => $workflow]) }}",
                            data: {
                                _token: "{{ csrf_token() }}",
                    ...
                    function loadSettings(type, element_id = 0, element) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.getElementSettings', ['workflow' => $workflow]) }}",
                            data: {
                                _token: "{{ csrf_token() }}",
                                type: type,
                                element_id: element_id,
                    ...
                    function loadContitions(type, element_id = 0, element) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('laravel-mail.workflows.getElementConditions', ['workflow' => $workflow]) }}",
                            data: {
                                _token: "{{ csrf_token() }}",
                                type: type,
                                element_id: element_id,
