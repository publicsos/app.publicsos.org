...
@section('content')
    ...
    @include('laravel-mail::messages.partials.nav')

    @component('laravel-mail::layouts.partials.actions')
        @slot('left')
            <form action="{{ route('laravel-mail.messages.index') }}" method="GET" class="form-inline">
                <div class="mr-2">
                    <input type="text" class="form-control" placeholder="Search..." name="search"
                           value="{{ request('search') }}">
                </div>

                @if(request()->route()->named('laravel-mail.messages.index'))
                    <div class="mr-2">
                ...
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('laravel-mail.messages.index') }}" class="btn btn-light">{{ __('Clear') }}</a>
                @endif
            </form>
        @endslot
    @endcomponent
        ...
        <div class="card-table table-responsive">
            <table class="table table-dark table-hover">
                ...
                <tr>
                    ...
                    <th>{{ __('Recipient') }}</th>
                    <th>{{ __('Status') }}</th>
                    @if(request()->route()->named('laravel-mail.messages.draft'))
                        <th>{{ __('Actions') }}</th>
                        <th>
                            <button class="btn btn-xs btn-light" id="select-all">Select All</button>
                            <form action="{{ route('laravel-mail.messages.send-selected') }}" method="post" id="send-selected-form" style="display: inline-block;">
                                @csrf
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @elseif($message->isAutomation())
                                    ...
                                    <a href="{{ route('laravel-mail.automations.show', $message->source->automation_step->automation_id) }}">
                            ...
                            @if(request()->route()->named('laravel-mail.messages.draft') &&  ! $message->sent_at)
                                <td>
                                    <form action="{{ route('laravel-mail.messages.send') }}" method="post" class="d-inline-block">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $message->id }}">
                                        <a href="{{ route('laravel-mail.messages.show', $message->id) }}" class="btn btn-xs btn-light">{{ __('Preview') }}</a>
                                        <button type="submit" class="btn btn-xs btn-light">{{ __('Send Now') }}</button>
                                    </form>
                                    <form action="{{ route('laravel-mail.messages.delete', $message->id) }}" method="post" class="d-inline-block delete-message">
                                        @csrf
                                        @method('delete')
